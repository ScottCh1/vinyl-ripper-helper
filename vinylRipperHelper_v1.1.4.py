...
def determineHtmlPageType(filepath):
  ...
  for table in tables:
    if 'class' in table.attrs:
      ...
      #print('DBG: Table type "%s" found in "%s"'% (table['class'], filepath))

    if len(classes) > 0:
      for aclass in classes:
        if 'tracklist_3QGRS' in aclass or 'tracklist_c5ftk' in aclass or 'tracklist_ZdQ0I' in aclass or 'tracklist_odjYc' in aclass or 'playlist' in aclass:
          return table['class']
...
def readAlbumLabelDataFromHtml(filepath, tabletype):
  ...
  # print('DBG: ROW="%s"' % (row))
    ...
    if 'data-track-position' in row.attrs:
      ...
      #print('DBG: TRACK POSITION = "%s"' % (track['pos']))
      found_pos = True
    cols = row.find_all('td')
    for col in cols:
      if 'class' in col.attrs: 
        if 'duration_2t4qr' in col['class'] or 'duration_GhhxK' in col['class'] or 'tracklist_track_duration' in col['class']:
          spans = col.find_all('span')
          track['time'] = spans[0].text
          found_time = True
        else:
          # this class variant has the duration as its text
          if 'duration_FhdYc' in col['class']:
            track['time'] = col.text
            found_time = True
      ...
      if len(spans) < 1:
        ...
        if 'trackTitleNoArtist_VUgUr' in col['class'] or 'trackTitle_loyWF' in col['class']:
          track['title'] = col.text
          found_title = True

        # The track title is in a span with a class (tracklist_track_title)
        if 'class' in spans[0].attrs and False == found_title:
          track['title'] = spans[0].text
        else: # The track time is in a span with no defined class
          if False == found_time and 'text' in spans[0]:
            track['time'] = spans[0].text
            found_time = True
    # Position is the last key added to the track element
    if 'pos' in track:
